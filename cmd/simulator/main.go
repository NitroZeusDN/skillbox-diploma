...
var smsFilename = "sms.csv"
var voiceFilename = "voice.csv"
var emailFilename = "email.csv"
var billingFilename = "billing.txt"
...
func main() {
	...
	listenAndServeHTTP(fmt.Sprintf(":%d", cfg.Simulator.Port))
...
func shuffleSmsData() {
	...
	err := ioutil.WriteFile(getFilapathByFilename(config.Get().TempDir+smsFilename), []byte(data), 0644)
	if err != nil {
...
func shuffleVoiceData() {
	...
	err := ioutil.WriteFile(getFilapathByFilename(config.Get().TempDir+voiceFilename), []byte(data), 0644)
	if err != nil {
		fmt.Printf("Error in write sms data: %s", err.Error())
	}
}
...
func shuffleEmailData() {
	...
	err := ioutil.WriteFile(getFilapathByFilename(config.Get().TempDir+emailFilename), []byte(data), 0644)
	if err != nil {
...
func shuffleBillingData() {
	...
	err := ioutil.WriteFile(getFilapathByFilename(config.Get().TempDir+billingFilename), []byte(data), 0644)
	if err != nil {
		fmt.Printf("Error in write sms data: %s", err.Error())
	}
}
